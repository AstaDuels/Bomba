...
lockBtn.Text                  = autoGrabLocked and "L" or "U"
...
collapseBtn.Text                   = "-"
collapseBtn.TextColor3             = Color3.fromRGB(80, 80, 80)
...
collapseStroke.Color = Color3.fromRGB(60, 60, 60)
...
-- Progress bar fill: pure black, no shine/sweep effects
...
fillGrad.Color = ColorSequence.new({
    ...
    ColorSequenceKeypoint.new(0.70, Color3.fromRGB(0, 0, 0)),
...
startBatAimbot = function()
    ...
    local function createPredictionSphere()
        ...
        predictionSphere.Material=Enum.Material.Neon; predictionSphere.Color=Color3.fromRGB(100,100,100); predictionSphere.Transparency=1
        local light=Instance.new("PointLight"); light.Color=Color3.fromRGB(100,100,100); light.Range=8; light.Brightness=2; light.Parent=predictionSphere
        local outline=Instance.new("Highlight"); outline.FillTransparency=1; outline.OutlineColor=Color3.fromRGB(80,80,80); outline.OutlineTransparency=1; outline.Parent=predictionSphere
...
local function reativarBypassTP()
    if not bypassTPEnabled then return end
    if bypassTPConn then
        bypassTPConn:Disconnect()
        bypassTPConn = nil
...
local buttonVisibility = {
    drop=true, tpDown=true, batV1=true, batV2=true,
...
local KB = {
    ...
    TPBypass      = {kb = Enum.KeyCode.B,           gp = nil}
}
...
UIS.InputBegan:Connect(function(input, gpe)
    ...
    elseif kbMatch(KB.TPBypass, kc) then
        toggleBypassTP()
        if setBypassTPVisual then setBypassTPVisual(bypassTPEnabled) end
        saveConfig()
    end
end)
...
local function buildMobileButtons()
    ...
    local buttonOrder = {
        ...
        {key="tpBypass",     label="TP BYPASS",       toggle=true},
    ...
    for i, def in ipairs(visibleOrder) do
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            if def.key == "tpBypass" then
                toggleBypassTP(); setOn(bypassTPEnabled)
                saveConfig(); return
            end
    ...
    if mobBtnRefs.tpBypass then mobBtnRefs.tpBypass(bypassTPEnabled) end
    if mobBtnRefs.carrySpeed then mobBtnRefs.carrySpeed(carrySpeedActive) end
    if mobBtnRefs.lagger then mobBtnRefs.lagger(laggerModeEnabled) end
...
local function loadConfig()
    ...
    if cfg.bypassTP ~= nil then bypassTPEnabled = cfg.bypassTP end
    ...
    applyKey(KB.TPBypass, cfg.tpBypassKey)
end
...
local function buildGui()
    ...
    setBypassTPVisual = mkToggle(sfCombat, "TP Bypass", function(on)
        bypassTPEnabled = on
        if on then startBypassTP() else stopBypassTP() end
        saveConfig()
    end, bypassTPEnabled)
